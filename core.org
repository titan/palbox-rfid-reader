...
#+begin_src c :noweb-ref mainloop
  ...
  int main(void) {
    ...
    uart_start();
    rfid_cache_init();
    core_state = core_transform_state(core_state, CORE_INVENTORY_EVENT, NULL);
    ...
    while (1) {
      if (systick > loop_timer) {
        ...
        if (core_state == CORE_INVENTORY_SCANNING_STATE || core_state == CORE_DIFFERENCE_SCANNING_STATE) {
          if (systick > rfid_timer) {
            core_state = core_transform_state(core_state, CORE_TIMEOUT_EVENT, NULL);
          }
        }
        if (diff == 0 && core_state == CORE_READY_STATE) {
          diff = 1;
          core_state = core_transform_state(core_state, CORE_DIFFERENCE_EVENT, NULL);
        }
      }
...
#+begin_src c :noweb-ref core-fsm
  ...
  void core_do_action(enum CORE_ACTION action, void * data) {
    switch (action) {
    case CORE_SWAP_AND_SCAN_ACTION: {
    ...
    case CORE_STOP_SCAN_ACTION:
      rfid_stop();
      break;
    case CORE_STOP_SCAN_NEWLINE_CALCULATE_DIFFERENCE_ACTION: {
      rfid_stop();
